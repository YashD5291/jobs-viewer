...
export default function GlassdoorJobsPage() {
  ...
  const [filters, setFilters] = useState<FilterOptions>({
    isRemote: false,
    site: "glassdoor", // Pre-set to Glassdoor
  });
    ...
    async (page: number, limit: number, searchTerm: string, isRemote: boolean) => {
      ...
      try {
        const response = await fetchJobs(
          ...
          searchTerm,
          isRemote,
          "glassdoor" // Always filter by Glassdoor
        );
        if (response.success) {
          setJobs(response.data.jobs);
          setPagination(response.data.pagination);
        } else {
          setError("Failed to load Glassdoor jobs");
        }
      } catch (err) {
        setError("An error occurred while fetching Glassdoor jobs");
        console.error(err);
      } finally {
        setIsLoading(false);
      }
  ...
  const handleFilterChange = useCallback((newFilters: FilterOptions) => {
    // Ensure site is always glassdoor
    const updatedFilters = { ...newFilters, site: "glassdoor" };
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="space-y-4">
            ...
            <FilterBar 
              ...
              disableSiteFilter={true} /* Disable site filter since we're on Glassdoor page */
            />
          </div>
        </div>
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-medium text-gray-900">Glassdoor Jobs</h2>
          </div>
