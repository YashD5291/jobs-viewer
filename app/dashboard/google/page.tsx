...
export default function GoogleJobsPage() {
    ...
    async (page: number, limit: number, searchTerm: string, isRemote: boolean) => {
      ...
      try {
        const response = await fetchJobs(
          ...
          searchTerm,
          isRemote,
          "google" // Always filter by Google
        );
        if (response.success) {
          setJobs(response.data.jobs);
          setPagination(response.data.pagination);
        } else {
          setError("Failed to load Google jobs");
        }
      } catch (err) {
        setError("An error occurred while fetching Google jobs");
        console.error(err);
      } finally {
        setIsLoading(false);
      }
  ...
  const handleFilterChange = useCallback((newFilters: FilterOptions) => {
    // Ensure site is always google
    const updatedFilters = { ...newFilters, site: "google" };
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <div className="space-y-4">
            ...
            <FilterBar 
              ...
              disableSiteFilter={true} /* Disable site filter since we're on Google page */
            />
          </div>
        </div>
        ...
        <div className="bg-white shadow-md rounded-lg overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-medium text-gray-900">Google Jobs</h2>
          </div>
